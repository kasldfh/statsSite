...
<div class="row">
      ...
      <div class="box-body no-padding">
        <table class="table table-condensed" id="cssTable">
          <tbody><tr>
            ...
            <th style="width: 10px; text-align: center;">Average Uplinks</th>
            <th style="width: 10px; text-align: center;">Total Uplinks</th>
            <th style="width: 10px; text-align: center;">Maps</th>
          </tr>
          <?php $i=1; ?>
          @foreach($uplinkPlayers as $player)
          <tr>
            <td><b>{{$i}}</b></td>
            <td>
            ...
            <td><span class="badge bg-green">{{$player->getUplinkAverage()}}</span></td>
            <td><span class="badge bg-green">{{$player->getUplinkTotal()}}</span></td>
            <td><span class="badge bg-green">{{$player->getUplinkMapCount()}}</span></td>
          </tr>
          <?php $i++; ?>
          @endforeach
  ...
  <div class="col-md-6">
    <div class="box">
      ...
      <div class="box-header">
        <h3 class="box-title">Uplink K/D</h3>
      <div class="box-tools pull-right">
        <button class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
      </div>
      </div><!-- /.box-header -->
      ...
      <div class="box-body no-padding">
        <table class="table table-condensed" id="cssTable">
          ...
          <?php $i=1; ?>
          @foreach($uplinkKDPlayers as $player)
          <tr>
            <td><b>{{$i}}</b></td>
            <td>
              @if(!is_null($player->photo_url))
            ...
            <td><!-- <img class="team_img" src='http://hydra-media.cursecdn.com/cod.gamepedia.com/4/48/Optic.png?version=55115f8ad615910a2ed282dac9a39edc' height='20'/>  --> <b>{{$player->alias}}</b></td>
            <td><span class="badge bg-green">{{$player->getUplinkKD()}}</span></td>
            <td><span class="badge bg-green">{{$player->getUplinkMapCount()}}</span></td>
          </tr>
          <?php $i++; ?>
          @endforeach
